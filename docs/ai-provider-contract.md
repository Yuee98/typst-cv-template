...
- `attempt_v2` 与 `route_snapshot_v1` zero-child release 不把 `p_metadata.attempt_count` 当事实源，也不能用它覆盖 parent；只有真正 route-schema-NULL V1 保留既有 metadata write。对 `child row count > 0` 的 attempt-backed `attempt_v2` settlement，request 与 ordered attempts均锁定后必须验证 parent `attempt_count = child row count`，且 attempt_no exact set只能是 `{1}` 或 `{1,2}`；前述 zero-child release单独要求 count=0 + empty set，不适用 nonempty-set rule。任何 drift 在 mutation前以 `SERVICE_UNAVAILABLE` fail closed。
