...
| I05 runtime/legal evidence | in progress, dark | compiled capability catalog, immutable target bindings, sealed bilingual display schema and protected current identity are implemented; availability/consent rendering and deployment report admission remain dark |
| I06–I10 | pending | dependent on the runtime/legal evidence contracts |
...
I05 local progress (2026-09-04): migration `20260904002000_runtime_legal_evidence_v2.sql` adds the exact code-capability intersection, immutable profile/price/provider/runtime/legal target bindings, a sealed text-only bilingual disclosure schema, and a protected v2 current legal identity initialized from the unchanged v1 value. The v2 execution snapshot now returns and strictly decodes that evidence, rejects crossed tuples, and completes a synthetic local reservation/attempt/settlement without provider traffic. Production v2 admission remains disabled until the remaining disclosure/consent path and I06 deployment report producer are present.

Migration `20260904001000_provider_execution_v2_lifecycle.sql` adds service-role-only versioned snapshot/start entry points without modifying the v1 routines. A v2 attempt freezes endpoint, credential environment name, runtime build ID and binding-manifest revision before transmission admission; exact replays require the same provenance. The application uses the versioned snapshot wrapper, while the production v2 runtime target resolver remains deliberately empty. Local evidence at this checkpoint: 1,817 unit tests passed with one existing skip; typecheck and lint passed; a clean local migration reset completed; and 104 focused real-DB tests passed across the v1 compatibility and v2 execution/evidence suites. The CFG fresh-reset gates remain an exact-head requirement before handoff.
