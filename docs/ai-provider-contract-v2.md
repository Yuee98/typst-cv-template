...
Deploy the old/new renderer before changing current. Prepare both new-route and rollback candidates under the new bundle; create equivalent v2 profile/price successors where legacy targets cannot legally bind the new runtime. New-current validation, activation, pointer/readback and a separate reopen follow. Old policies remain history; prior frozen reservations are not rebound or reinterpreted by new-reservation consent rules.

A legal-current switch is a migration-owned DB-operator transition, rather than an Admin UI or ordinary Admin RPC. `current_ai_terms_version()` is migration-pinned and part of the authority receipt: with the gate off and the pointer cleared, the migration atomically selects the sealed bundle, advances the protected v2 current identity, updates the canonical expected authority definition, and stamps a new authority epoch before normal activation resumes. This prevents a browser mutation from changing the legal predicate that admits live requests.
