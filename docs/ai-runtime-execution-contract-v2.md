...
Portable vectors are added under `web/test/fixtures/admin-contract-v2.json` and successor execution fixtures. Negative vectors must exercise coherent tuple substitution, extra/missing fields, same adapter config with different destination/profile, model ID propagation, future bundle versus live gate, and stale report/control generation. Synthetic test models confer no hosted compatibility evidence.
