...
Provider/profile versions, exact price versions, routing policy, legal bundle, and the immutable versioned runtime-contract ID are **DB-authoritative snapshots**. The server freezes that exact route before transmission. For v2 profiles, the DB version owns the endpoint, credential environment-variable name, and model ID; compiled code constrains the adapter and destination policy, while a reviewed deployment manifest binds each credential name to its exact Provider, recipient, and origin. Environment variables only supply credentials for those bindings. They are not a provider or model selector. Do not add `AI_PROVIDER`, `AI_MODEL`, or `AI_BASE_URL` as application routing variables: doing so would bypass the DB validation, auditing, canary, and legal gates.

`DEEPSEEK_API_KEY` and `MIMO_API_KEY` remain the legacy v1 server-only aliases. The current seeded v2 bindings use `AI_PROVIDER_KEY_DEEPSEEK_PRIMARY` and `AI_PROVIDER_KEY_MIMO_PRIMARY`; v2 also requires `AI_RUNTIME_BUILD_ID` and the non-secret `AI_PROVIDER_BINDING_MANIFEST` to match a reviewed deployment. The initial route does not become enabled merely because a key exists; it still requires an approved active DB profile and policy. `OPENROUTER_API_KEY` is a future optional v1 alias only, remains outside the initial route, and requires a separately reviewed exact upstream, profile, disclosure, and activation. No provider key may enter browser bundles, the database, application logs, or ledger/error payloads.
...
| `AI_RUNTIME_BUILD_ID` | Reviewed Preview build ID | Reviewed Production build ID | Server-only v2 runtime identity; must match the registered deployment |
| `AI_PROVIDER_BINDING_MANIFEST` | Canonical reviewed Preview manifest JSON | Canonical reviewed Production manifest JSON | Non-secret v2 binding identity; exact Provider/recipient/origin bindings only |
| `AI_PROVIDER_KEY_DEEPSEEK_PRIMARY` / `AI_PROVIDER_KEY_MIMO_PRIMARY` | Non-production keys for approved Preview v2 profiles; otherwise leave unset | Separate production keys for approved Production v2 profiles | Sensitive server-only v2 bindings; names must match the DB profile and reviewed manifest |
| `DEEPSEEK_API_KEY` / `MIMO_API_KEY` | Non-production keys while an approved Preview v1 profile may run; otherwise leave unset | Separate production keys while an approved Production v1 profile may run | Sensitive legacy v1 aliases; keys alone do not activate routing |
