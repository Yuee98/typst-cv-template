-- CFG-005 audit projection: successor lifecycle events are immutable domain
-- evidence returned by Admin mutations. Expose only their explicit safe fields
-- through the existing protected read boundary; metadata remains private.
begin;
...
begin
  ...
  when 'audit' then $query$
    ...
    union all
    select event.id,event.operation || ' ' || event.runtime_contract_id as search_text,
      jsonb_build_object(
        'id',event.id,'occurredAt',event.occurred_at,
        'eventSchemaVersion','config_lifecycle_event_v2',
        'eventType',event.operation,'source','config_lifecycle',
        'sourceId',event.id,
        'operationId',committed.id,
        'correlationAuditId',committed.domain_audit_id,
        'operation',event.operation,'actor',event.actor,
        'targetId',event.target_id,'reason',event.reason,
        'runtimeContractId',event.runtime_contract_id,
        'validationReportIds',to_jsonb(event.validation_report_ids),
        'codeCapabilityId',event.code_capability_id,
        'codeCapabilitySha256',event.code_capability_sha256,
        'change',jsonb_strip_nulls(jsonb_build_object(
          'fromStatus',event.metadata->>'fromStatus',
          'toStatus',event.metadata->>'toStatus',
          'fromValidTo',event.metadata->>'fromValidTo',
          'toValidTo',event.metadata->>'toValidTo',
          'successorPriceVersionId',event.metadata->>'successorPriceVersionId',
          'fromRetiredAt',event.metadata->>'fromRetiredAt',
          'toRetiredAt',event.metadata->>'toRetiredAt'
        )))
      from public.admin_config_lifecycle_events_v2 event
      left join lateral (
        select committed.id,committed.domain_audit_id
        from public.admin_committed_operations committed
        where committed.committed_result->>'lifecycleAuditId'=event.id::text
        order by committed.committed_at desc
        limit 1
      ) committed on true
  $query$
  else null end;
...
begin
  ...
  set definition_sha256=encode(extensions.digest(
    replace(replace(pg_catalog.pg_get_functiondef(
      'public.admin_records_query_v1(text)'::regprocedure
    ),chr(13)||chr(10),chr(10)),chr(13),chr(10)),
    'sha256'
  ),'hex')
  where expected.signature='public.admin_records_query_v1(text)';
  if not found then
    raise exception 'RUNTIME_AUTHORITY_EXPECTED_MISSING' using errcode='23514';
  end if;
end;
$$;
