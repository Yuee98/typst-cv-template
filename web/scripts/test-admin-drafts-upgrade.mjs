// Run on the disposable PR40 predecessor before applying the next migration.
// All scenarios, including candidate DDL, roll back.
import { readFileSync } from "node:fs";
import { randomUUID } from "node:crypto";
import { spawnSync } from "node:child_process";

const container = process.env.ADMIN_DRAFT_UPGRADE_CONTAINER ?? "supabase_db_typst-cv-template";
if (!["supabase_db_typst-cv-template", "supabase_db_typst-cv-drafts-review"].includes(container)) {
  throw new Error("Requires the owned local predecessor fixture");
}
const migration = readFileSync(new URL("../../supabase/migrations/20260910000000_admin_drafts_before_cutover.sql", import.meta.url), "utf8");
...
const changed = "'admin_get_context_v1','admin_update_provider_defaults_v1','admin_create_provider_profile_v1','admin_create_profile_version_v2','admin_create_price_version_v1'";
const added = "'admin_create_routing_policy_draft_v1'";
...
owner(`do $$ begin
  if to_regprocedure('public.admin_bootstrap_v2(uuid,text,text)') is null
    or to_regprocedure('public.admin_create_routing_policy_draft_v1(text,text,text,integer,jsonb,uuid,text,text,text,uuid)') is not null
    or exists(select 1 from public.admin_environment) or exists(select 1 from public.admin_principals) then
    raise exception 'Requires an uninitialized local PR40 predecessor; never overwrite operator state';
  end if;
end; $$;`);
...
for (const mode of ["uninitialized", "legacy-ai-on", "jwt"]) {
  ...
  owner(`begin;
    ...
    do $verify$ begin
      ...
      if not pg_catalog.has_function_privilege('authenticated','public.admin_create_routing_policy_draft_v1(text,text,text,integer,jsonb,uuid,text,text,text,uuid)','EXECUTE')
        or pg_catalog.has_function_privilege('anon','public.admin_create_routing_policy_draft_v1(text,text,text,integer,jsonb,uuid,text,text,text,uuid)','EXECUTE')
        or pg_catalog.has_function_privilege('service_role','public.admin_create_routing_policy_draft_v1(text,text,text,integer,jsonb,uuid,text,text,text,uuid)','EXECUTE') then
  ...
  console.log(`Admin drafts upgrade: ${mode} passed (authority unchanged, rolled back)`);
}
