/**
 * Hard-fail CFG-002 seed gate. The target is proved loopback both before and
 * after reset; this command never falls back to the ordinary optional DB run.
 */
...
export async function runCfg002FreshReset({
  env = process.env,
  ...
  const fail = (message) => {
    errorLogger(`[test:db:cfg002-fresh] ERROR: ${message}`);
    return 1;
  };
  ...
  logger("[test:db:cfg002-fresh] resetting verified loopback Supabase");
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  process.exitCode = await runCfg002FreshReset();
}
