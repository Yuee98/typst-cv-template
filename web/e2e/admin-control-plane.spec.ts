...
test("prepares a new identity, its first version and its first price through the UI", async ({ page }) => {
  await withLegacyDraftPreparation(async () => {
    ...
    await page.goto("/en/admin/providers?search=no-such-provider-for-create");
    await expect(page.getByText("No records found.")).toBeVisible();
    await page.getByRole("button", { name: "Create new", exact: true }).click();
    const providerForm = panel("Create new");
    const providerKey = `test.ui.custom.${Date.now()}`;
    const preparedModel = `prepared-new-model-${Date.now()}`;
    await providerForm.getByLabel("Provider key", { exact: true }).fill(providerKey);
    await providerForm.getByLabel("Display name", { exact: true }).fill("UI custom provider");
    await providerForm.getByLabel("Recipient key", { exact: true }).fill("ui-custom");
    await expect(providerForm.getByLabel("Adapter", { exact: true })).toBeEnabled();
    await providerForm.getByLabel("Adapter", { exact: true }).selectOption("deepseek_chat_v1");
    await providerForm.getByLabel("Default endpoint", { exact: true }).fill("https://example.test/chat/completions");
    await providerForm.getByLabel("Default credential environment variable", { exact: true }).fill("AI_PROVIDER_KEY_UI_UNUSED");
    await providerForm.getByLabel("Default model", { exact: true }).fill("ui-test-model");
    await providerForm.getByLabel("Reason", { exact: true }).fill("create provider from empty search");
    const providerResponse = page.waitForResponse(res => res.url().endsWith("/api/admin") && res.request().method() === "POST");
    await providerForm.getByRole("button", { name: "Create new", exact: true }).click();
    expect((await providerResponse).status()).toBe(200);
    await expect(page).toHaveURL(new RegExp("/en/admin/providers/[0-9a-f-]{36}$"));
    const createdProvider = { result: { providerId: new URL(page.url()).pathname.split("/").at(-1)! } };
    await page.goto("/en/admin/profiles?search=no-such-profile-for-create");
    await page.getByRole("button", { name: "Create new", exact: true }).click();
    await page.getByLabel("Providers — Search", { exact: true }).fill(providerKey);
    await expect(page.getByLabel("Providers", { exact: true }).locator(`option[value="${createdProvider.result.providerId}"]`)).toHaveCount(1);
    await page.getByLabel("Providers", { exact: true }).selectOption(createdProvider.result.providerId);
    ...
    await page.goto(`/en/admin/providers/${createdProvider.result.providerId}`);
    ...
    await version.getByPlaceholder("Model", { exact: true }).fill(preparedModel);
    ...
    let price = panel("Create first price for this version");
    ...
    await expect(price.getByText(`Profile version ID: ${profileVersionId}`, { exact: true })).toBeVisible();
    await page.goto("/en/admin/prices?search=no-such-price-for-create");
    await page.getByRole("button", { name: "Create new", exact: true }).click();
    await page.getByLabel("Profile version ID — Search", { exact: true }).fill(preparedModel);
    await expect(page.getByLabel("Profile version ID", { exact: true }).locator('option[value="' + profileVersionId + '"]')).toHaveCount(1);
    await page.getByLabel("Profile version ID", { exact: true }).selectOption(profileVersionId);
    price = panel("Create first price for this version");
    ...
    await page.goto("/en/admin/policies?search=no-such-policy-for-create");
    await page.getByRole("button", { name: "Create new", exact: true }).click();
    const policy = panel("Create version");
    await policy.getByLabel("Policy key", { exact: true }).fill('test.ui.policy.' + Date.now());
    const defaults = policy.getByRole("group", { name: "Default route", exact: true });
    await defaults.getByLabel("Profile version ID — Search", { exact: true }).fill(preparedModel);
    await expect(defaults.getByLabel("Profile version ID", { exact: true }).locator('option[value="' + profileVersionId + '"]')).toHaveCount(1);
    await defaults.getByLabel("Profile version ID", { exact: true }).selectOption(profileVersionId);
    await expect(defaults.getByLabel("Price version ID", { exact: true })).toBeEnabled();
    await defaults.getByLabel("Price version ID", { exact: true }).selectOption(createdPrice.result.priceVersionId);
    await policy.getByPlaceholder("Legal bundle", { exact: true }).fill("ui-future-legal");
    await expect(policy.getByLabel("Runtime contract", { exact: true })).toBeEnabled();
    await policy.getByLabel("Runtime contract", { exact: true }).selectOption({ index: 1 });
    await policy.getByPlaceholder("Reason", { exact: true }).fill("prepare structured default route");
    await policy.getByRole("button", { name: "Add time window", exact: true }).click();
    const windowForm = policy.getByRole("group", { name: "Time window 1", exact: true });
    await expect(policy.getByRole("button", { name: "Create version", exact: true })).toBeDisabled();
    await windowForm.getByLabel("End (HH:mm, up to 24:00)", { exact: true }).fill("24:00");
    await windowForm.getByLabel("Sat", { exact: true }).check();
    await windowForm.getByLabel("Profile version ID — Search", { exact: true }).fill(preparedModel);
    await expect(windowForm.getByLabel("Profile version ID", { exact: true }).locator('option[value="' + profileVersionId + '"]')).toHaveCount(1);
    await windowForm.getByLabel("Profile version ID", { exact: true }).selectOption(profileVersionId);
    await expect(windowForm.getByLabel("Price version ID", { exact: true })).toBeEnabled();
    await windowForm.getByLabel("Price version ID", { exact: true }).selectOption(createdPrice.result.priceVersionId);
    await policy.getByRole("button", { name: "Add time window", exact: true }).click();
    await policy.getByRole("group", { name: "Time window 2", exact: true }).getByRole("button", { name: "Remove window" }).click();
    const createdPolicy = await submit(policy);
    expect(createdPolicy.result).toMatchObject({ version: 1, status: "draft" });
    await page.goto('/en/admin/policies/' + createdPolicy.result.policyVersionId);
    await expect(page.getByRole("heading", { name: "Default route", exact: true })).toBeVisible();
