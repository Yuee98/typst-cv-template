...
export const aiTermsDocument: LegalDocument = {
  ...
  intro: [
    `本 AI 服务条款管辖你对 ${SERVICE_NAME}（${SERVICE_WEBSITE}）AI 润色功能的使用，与使用条款和隐私政策共同适用。`,
    "使用 AI 润色前，你必须先同意本条款。我们会记录你的同意（用户 ID、条款版本与时间戳），以便证明已获得你的授权。",
  ],
  sections: [
    {
      heading: "AI 润色功能",
      body: [
        ...
        "当你确认润色请求时，你选中的文本以及你选择包含的上下文会经我们的服务器转发至第三方 AI 服务进行处理。",
      ],
    },
    {
      heading: "发送的内容",
      body: [
        "我们不会主动发送简历页眉中的姓名、邮箱和电话。但如果你选中的正文及上下文本身包含个人信息，这些信息仍会作为请求的一部分发送至 AI 服务。",
        "每次请求前，功能会如实展示将要发送的内容。请仔细检查披露内容，移除或避免包含你不愿分享的信息。",
      ],
    },
    {
      heading: "我们存储的内容",
      body: [
        ...
        "我们会保留每次请求的元数据日志，例如时间戳、用户 ID、功能设置、请求次数、token 用量、延迟以及状态或错误码。这些日志绝不包含你的简历正文、AI 输出或风格指令。",
        "元数据日志最多保留 90 天，之后会被删除。你对本 AI 服务条款的同意记录会保留至你删除账户为止。",
      ],
    },
    {
      heading: "第三方 AI 服务",
      body: [
        "AI 润色由第三方 AI 服务 DeepSeek 提供支持。你发送的内容由 DeepSeek 按其自身条款和隐私政策处理，这不受我们控制。",
        "对于 DeepSeek 如何留存、使用或删除其接收的内容（包括是否将其用于模型训练），我们不做任何陈述或保证。如果你对此有顾虑，请勿在润色请求中包含敏感信息，或不要使用本功能。",
    ...
    },
    {
      ...
      body: [
        ...
        "AI 润色设有使用限制，例如每日请求次数限制和频率限制。对于滥用、过度使用或给服务带来风险的情况，我们可能会暂停访问权限。",
      ],
    },
    {
      heading: "加密与 AI 润色",
