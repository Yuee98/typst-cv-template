...
function candidate(){return {
 schemaVersion:"admin_runtime_readback_candidate_v4",environment:"local",
...
function report(){
 ...
 return {...base,schemaVersion:"admin_runtime_readback_v4",reportId:"55555555-5555-4555-8555-555555555555",checkedAt:checkedAt.toISOString(),expiresAt:new Date(checkedAt.getTime()+9*60_000).toISOString(),reportSha256:"a".repeat(64)};
}
...
describe("current-runtime configuration readback",()=>{
 it("observes code and credentials before recording the exact closing cycle",async()=>{
  ...
  expect(rpc.mock.calls[1]).toEqual(["record_admin_runtime_readback_v3",{
   p_environment:"local",p_project_ref:null,p_policy_version_id:input.policyVersionId,p_validation_report_ids:input.validationReportIds,
