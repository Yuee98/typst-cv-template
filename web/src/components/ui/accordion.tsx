...
export function AccordionItem({
  ...
  return (
    <AccordionPrimitive.Item
      ref={ref}
      className={cn("accordion-item border-b border-border", className)}
      {...props}
    />
  );
}
  ...
  return (
    <AccordionPrimitive.Header className="flex flex-1">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "accordion-trigger flex min-h-11 w-full flex-1 items-center justify-between gap-3 py-2 text-left text-sm font-semibold text-foreground outline-none transition-colors hover:text-foreground-muted [&[data-state=open]>svg]:rotate-180",
          className,
        )}
        {...props}
      >
